...
def wav_to_frequencies(filename: str, window: int) -> list:
    """Convert WAV to a list of frequencies

    Args:
        filename (str): WAV filename
        window (int): resolution of the time window for frequency analysis in miliseconds

    Returns:
        list of (duration, frequency) tuples
    """

    fs, data = wavfile.read(filename)
    data = data[: MAX_LEN * fs]
    # Window size and overlap definition for the spectral analysis
    w = int(fs / 1000 * window)
    overlap = w // 2
...
freql = wav_to_frequencies(args.file, args.window)
